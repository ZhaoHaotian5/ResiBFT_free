...
Block::Block(Hash previousHash, unsigned int size, Transaction transactions[NUM_TRANSACTIONS])
{
	this->set = true;
...
std::string Block::toPrint()
{
	...
	for (int i = 0; i < NUM_TRANSACTIONS && i < this->size; i++)
	{
...
bool Block::operator==(const Block &data) const
{
	...
	for (int i = 0; i < NUM_TRANSACTIONS && i < this->size; i++)
	{
		if (!(this->transactions[i] == data.transactions[i]))
		{
			return false;
		}
	}
