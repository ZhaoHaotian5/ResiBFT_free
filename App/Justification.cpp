...
{
	if (this->roundData.getProposeView() == 0 && this->roundData.getJustifyView() == 0 && this->roundData.getPhase() == PHASE_NEWVIEW_FAST && this->signs.getSize() == 0)
	{
		return true;
	}
	else
...
{
	if (this->roundData.getJustifyHash().getSet() && this->roundData.getPhase() == PHASE_NEWVIEW_FAST && this->signs.getSize() == 1)
	{
		return true;
	}
	else
...
{
	if (this->roundData.getProposeHash().getSet() && this->roundData.getPhase() == PHASE_PREPARE_FAST && this->signs.getSize() == quorumSize)
	{
		return true;
	}
	else
