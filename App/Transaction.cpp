...
bool Transaction::operator==(const Transaction &data) const
{
	...
	for (int i = 0; i < NUM_PAYLOAD_SIZE; i++)
	{
		if (!(this->transactionData[i] == data.transactionData[i]))
		{
			return false;
		}
	}
...
bool Transaction::operator<(const Transaction &data) const
{
	if (this->clientId < data.clientId || this->transactionId < data.transactionId || this->transactionData < data.transactionData)
	{
		return true;
	}
	else
