...
`git clone https://github.com/ZhaoHaotian5/ResiBFT; cd ResiBFT`

`docker build -t resibft .`

We provided various options in `experiment.py`. Several default running commands in the Local Mode of ResiBFT is as following:
...
`python3 experiments.py --RunLocal --RunResiBFT --NumExperiments 50 --NumFaults 5 --ResiBFTFactor 1`
...
Several default running commands in the Server Mode of ResiBFT is as following:
...
`python3 experiments.py --RunInstance --RunResiBFT --NumExperiments 50 --NumFaults 10 --ResiBFTFactor 1`
...
- `--RunResiBFT` run ResiBFT protocol
- `--ResiBFTFactor` set the const factor of trusted replicas in ResiBFT
